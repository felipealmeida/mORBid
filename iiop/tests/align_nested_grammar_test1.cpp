...
int main()
{
  ...
  typedef fusion::vector4
    <std::vector<service_context_f>, unsigned int, unsigned int
     , unsigned short> nested_attribute_type;
  typedef fusion::vector6
    <std::vector<service_context_f>, unsigned int, unsigned int
     , unsigned short, nested_attribute_type, unsigned int> attribute_type;
  ...
  typedef boost::fusion::vector3<unsigned char
                                 ...
                                 , unsigned int>
    attribute_test_type;
  nested_attribute_type nested_attribute(service_contexts, 10, 20, 30);
  ...
  typedef morbid::giop::grammars::reply_1_0<iiop::generator_domain
    ...
    nested_generator_reply_header_grammar;
  typedef morbid::giop::grammars::reply_1_0<iiop::generator_domain
                                            , output_iterator_type
                                            , attribute_type>
    generator_reply_header_grammar;
  nested_generator_reply_header_grammar nested_generator_reply_header
    (giop::ushort_);
  generator_reply_header_grammar generator_reply_header
    (giop::ushort_ & nested_generator_reply_header & giop::ulong_);
                     ...
                     , attribute))
  {
    ...
    assert(output.size() == 4 /* octet + padding*/
           + 28 /*sc*/ + 8 /* 2ulongs */ + 2 /* ushort */
           + 2 /* padding */
           + 28 /*sc*/  + 8 /* 2ulongs */ + 4 /* short + pad */
           + 4 /* ulong */ + 8 /* endianness + ulong */);
    ...
    parser_reply_header_grammar nested_parser_reply_header
      (giop::ushort_);
    parser_reply_header_grammar parser_reply_header
      (giop::ushort_ & nested_parser_reply_header & giop::ulong_);
